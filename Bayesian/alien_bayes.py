...
class SimpleSampler(object):
    ...
    def get_prob(self, query_vals, num_samples):
        ...
        count = 0
        flag = True
        dict = self.generate_samples(num_samples)
        for e in dict:
            flag = True
            for key, value in query_vals.items():
                if e.get(key) != value:
                    flag = False
                    break
            if flag:
...
class RejectionSampler(SimpleSampler):
    ...
    def get_prob(self, query_vals, evidence_vals, num_samples):
        ...
        """
        def eval(element, evid):
            bool_val = True
            for elem, val in evid.items():
                if element.get(elem) != val:
                    bool_val = False
                    break
            return bool_val
        count = 0
        ...
        if len(evidence_vals) != 0:
            dict[:] = [e for e in dict if eval(e, evidence_vals)]
        for e in dict:
            flag = True
            for key, value in query_vals.items():
                if e.get(key) != value:
...
class LikelihoodWeightingSampler(SimpleSampler):
    ...
    def get_prob(self, query_vals, evidence_vals, num_samples):
        ...
        Returns: empirical conditional probability of query values given evidence
        """
        dict = self.generate_samples(num_samples, evidence_vals)
        numerator = 0
        denominator = 0
        flag = True
        for e in dict:
